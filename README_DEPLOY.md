...
- `SOLANA_RPC_URL` (**server-side only — set this one**) - The scanner's Solana RPC endpoint, and where a keyed provider URL belongs (Helius, QuickNode, Triton). Read by `netlify/functions/solana-rpc.mjs`, which proxies the scanner's read-only JSON-RPC calls behind a method allowlist and a per-IP rate limit, by the `khan-holders-*` indexer, and by `verify-solana-payment.mjs`. Because the payment verifier depends on it, leaving it blank is not merely a performance choice: the unkeyed `api.mainnet-beta.solana.com` fallback is throttled hard enough that a rate-limited `getTransaction` looks exactly like a transaction that never happened, and a customer who really paid is told their payment could not be verified. Never expose as a `VITE_` variable.
...
> **`VITE_SOLANA_RPC_URL` is retired.** It was inlined into the client bundle by Vite, so setting it to a keyed provider URL - as earlier notes recommended, because the public endpoint throttles browser traffic - published that key to every visitor. If it is still set in the Netlify UI, **delete it, and rotate the provider key it contained**, then put the keyed URL in `SOLANA_RPC_URL` instead.
>
> This note previously claimed the variable was "no longer read by any code". That was **false**: `verify-solana-payment.mjs` read it, and read *only* it, so an operator who followed the instruction above downgraded live payment verification to the throttled public endpoint. The verifier now prefers `SOLANA_RPC_URL` and keeps the retired name as a last resort purely so a half-migrated deployment does not break mid-cutover. **Set `SOLANA_RPC_URL` before deleting `VITE_SOLANA_RPC_URL`.**
...
- `LIFECYCLE_UNSUBSCRIBE_SECRET` (optional, server-side only) - Signs the unsubscribe and resubscribe capability links in lifecycle emails (`_lifecycleToken.mjs`). Falls back to `AUTH_SECRET`, so no action is needed if that is already set. With **neither** set, the mailer still sends but every opt-out link answers “preferences cannot be changed right now” — verify this before switching on `RESEND_API_KEY`, by opening `/unsubscribe` on the live site and checking it does not say “Unavailable”. Never a `VITE_` variable.
