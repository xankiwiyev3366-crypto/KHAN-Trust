...
// ── The core rule: 3 free scans a day, then a block ───────────────────────────

test('an anonymous user gets exactly 3 scans, then is blocked', async () => {
  reset();
  const first = parse(await anon('2.2.2.2', { consume: true }));
  assert.deepEqual([first.allowed, first.remaining], [true, 2]);
  assert.equal(parse(await anon('2.2.2.2', { consume: true })).remaining, 1);
  const third = parse(await anon('2.2.2.2', { consume: true }));
  assert.deepEqual([third.allowed, third.remaining, third.limitReached], [true, 0, true]);

  const fourth = parse(await anon('2.2.2.2', { consume: true }));
...
test('two different IPs are counted independently', async () => {
  reset();
  await anon('3.3.3.3', { consume: true });
  await anon('3.3.3.3', { consume: true });
  await anon('3.3.3.3', { consume: true });
...
test('a signed-in user is counted by account, not IP — a new IP does not reset it', async () => {
  ...
  await asUser(user, { consume: true }, '5.5.5.5');
  await asUser(user, { consume: true }, '5.5.5.5');
  await asUser(user, { consume: true }, '5.5.5.5');
